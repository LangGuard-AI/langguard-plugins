...
policy reason), or **ASK** (a human must approve this specific call). Decisions
...
ASK verdict rendered as `force_ask`: policy requires a human to look at this exact
tool call, so the prompt appears even if the user previously chose Always-Allow for the
tool. Repeated prompts on ask-tier tools are intentional, not a bug. If a call is
